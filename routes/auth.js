...
router.post('/login', async (req, res) => {
  try {
    ...
    await db.prepare('INSERT INTO refresh_tokens (user_id,token,expires_at) VALUES (?,?,?)').run(user.id, refreshToken, exp);
...
router.post('/refresh', async (req, res) => {
  ...
  try {
    ...
    const db     = getDb();
    const stored = await db.prepare('SELECT * FROM refresh_tokens WHERE token=? AND user_id=?').get(refreshToken, dec.id);
    if (!stored || new Date(stored.expires_at) < new Date())
      return res.status(401).json({ error: 'Token expiré' });
    const user = await db.prepare('SELECT * FROM users WHERE id=?').get(dec.id);
...
router.post('/logout', authenticateToken, async (req, res) => {
  const { refreshToken } = req.body;
  if (refreshToken) await getDb().prepare('DELETE FROM refresh_tokens WHERE token=?').run(refreshToken);
  res.json({ message: 'Déconnexion réussie' });
});

router.get('/me', authenticateToken, async (req, res) => {
  res.json(await getDb().prepare('SELECT id,name,email,role,department,phone,created_at FROM users WHERE id=?').get(req.user.id));
});

router.get('/users', authenticateToken, async (req, res) => {
  if (!['admin','director'].includes(req.user.role)) return res.status(403).json({ error: 'Accès refusé' });
  res.json(await getDb().prepare('SELECT id,name,email,role,department,phone,is_active,created_at FROM users').all());
});
...
router.put('/users/:id', authenticateToken, async (req, res) => {
  ...
  const db = getDb();
  await db.prepare("UPDATE users SET name=COALESCE(?,name),department=COALESCE(?,department),phone=COALESCE(?,phone),role=COALESCE(?,role),is_active=COALESCE(?,is_active),updated_at=datetime('now') WHERE id=?")
    .run(name, department, phone, role, is_active, req.params.id);
  res.json(await db.prepare('SELECT id,name,email,role,department,phone,is_active FROM users WHERE id=?').get(req.params.id));
});
